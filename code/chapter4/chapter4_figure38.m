%This Matlab script can be used to reproduce Figure 4.38 in the textbook:
...
beamforminggain = zeros(length(varphi),length(Mh));
...
for i = 1:length(varphi)
    ...
    for m = 1:length(Mh)
        ...
        if factor == 0
            ...
            beamforminggain(i,m) = abs(sin(Mh(m)*factor)/sin(factor))^2/Mh(m)*Mv(m);
        end
