%This Matlab script can be used to reproduce Figure 8.8 in the textbook:
...
for trial = 1
    ...
    for t = 1:length(angleGrid)
        BF(t,trial,1) = abs(conj(exp(-1i*pi*(0:(M-1))*sin(angleGrid(t))))*Y2*exp(-1i*pi*(0:(M-1))*sin(angleGrid(t))).');
    end
    ...
    for t = 1:length(angleGrid)
        BF(t,trial,2) = 1/abs((conj(exp(-1i*pi*(0:(M-1))*sin(angleGrid(t))))/Y2)*exp(-1i*pi*(0:(M-1))*sin(angleGrid(t))).');
    end
...
ax = gca;
set(ax, 'XTick', [ 1/6, 1/5,  1/3]);
xticklabels({'$\frac{\pi}{6}$','$\frac{\pi}{5}$', '$\frac{\pi}{3}$'});
ylim([-10 1]);
xlim([1/8 1/4]);
xlabel('Angle-of-arrival $\varphi$','Interpreter','latex');
ylabel('Normalized power spectrum [dB]','Interpreter','latex');
legend({'Conventional', 'Capon'},'Interpreter','latex');
set(gca,'fontsize',16);
