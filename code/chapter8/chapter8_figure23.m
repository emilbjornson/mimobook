%This Matlab script can be used to reproduce Figure 8.23 in the textbook:
...
for s = 1:length(SNRAll)
    for m = 1:length(MAll)
        ...
        noiseRealizations = sqrt(0.5)*(randn(1,numberOfsetups)+1i*randn(1,numberOfsetups));

        %Generate RCS realizations
        RCSRealizations =  sqrt(0.5)*(randn(1,numberOfsetups)+1i*randn(1,numberOfsetups));

        %Compute received signal
        receivedSignal = sqrt(rho*L)*M*RCSRealizations+noiseRealizations;

        %Compute the detection probability numerically
        Pd(m,s) = length(find(abs(receivedSignal).^2>=threshold))/numberOfsetups;
