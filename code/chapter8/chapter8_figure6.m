%This Matlab script can be used to reproduce Figure 8.6 in the textbook:
...
for t = 1:length(angleGrid)
    BF(t,trial,2) = 1./(real((conj(exp(-1i*pi*(0:(M-1))*sin(angleGrid(t))))/Y2)*exp(-1i*pi*(0:(M-1))*sin(angleGrid(t))).'));
end
