%This Matlab script can be used to reproduce Figure 8.7 in the textbook:
...
for trial = 1:numTrial
    ...
    for noiseSel = 1:2
        ...
        for l = 1:length(Lrange)
            ...
            for t = 1:length(angleGrid)
                BF(t,2) = 1/real((conj(exp(-1i*pi*(0:(M-1))*sin(angleGrid(t))))/Y2)*exp(-1i*pi*(0:(M-1))*sin(angleGrid(t))).')/M^2;
            end
            [~,maxind1] = max(BF(:,1));
            [~,maxind2] = max(BF(:,2));
...
legend({'Conventional, $\textrm{SNR}=0$\,dB', 'Conventional, $\textrm{SNR}=10$\,dB' 'Capon, $\textrm{SNR}=0$\,dB','Capon, $\textrm{SNR}=10$\,dB'},'Interpreter','latex');
set(gca,'YScale','log');
set(gca,'fontsize',16);
xlim([20 100])
ylim([7e-7 2e-4]);
